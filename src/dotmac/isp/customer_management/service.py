...
from dotmac.isp.customer_management.schemas import (
...
# Stub for geocoding - in split architecture, this would call a geocoding service
async def geocode_customer_address(
    customer_data: dict[str, Any], force: bool = False
) -> dict[str, float] | None:
    """
    Geocode customer address.

    NOTE: This is a stub. In production, implement actual geocoding via
    a shared service or external API (e.g., Google Maps, OpenStreetMap).
    """
    # Return None to skip geocoding in stub implementation
    return None


# Stub for event bus - in split architecture, events go via Platform API
class _EventBusStub:
    """Stub event bus that logs events instead of publishing."""
...
_event_bus_stub = _EventBusStub()


def get_event_bus() -> _EventBusStub:
    """Get the event bus stub."""
    return _event_bus_stub


# Webhook event enum stub
class WebhookEvent:
    """Webhook event types."""
...
class CustomerService:
    ...
    async def _calculate_segment_members(self, criteria: dict[str, Any], tenant_id: str) -> int:
        ...
        if "tier" in criteria:
            from dotmac.isp.customer_management.models import CustomerTier
